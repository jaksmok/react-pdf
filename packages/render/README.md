...
# @jaksmok-react-pdf/render

> React-pdf render engine

## How to install
```sh
yarn add @jaksmok-react-pdf/render
```

## How it works

```js
const render = require('@jaksmok-react-pdf/render');
const primitives = require('@jaksmok-react-pdf/primitives');
...
Mandatory field specifiying the type of the particular node. The full list of types can be found and imported from `@jaksmok-react-pdf/primitives`
...
const fs = require('fs');
const render = require('@jaksmok-react-pdf/render');
const pdfkit = require('@jaksmok-react-pdf/pdfkit');
