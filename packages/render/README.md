...
# @jakmsok-react-pdf/render

> React-pdf render engine

## How to install
```sh
yarn add @jakmsok-react-pdf/render
```

## How it works

```js
const render = require('@jakmsok-react-pdf/render');
const primitives = require('@jakmsok-react-pdf/primitives');
...
Mandatory field specifiying the type of the particular node. The full list of types can be found and imported from `@jakmsok-react-pdf/primitives`
...
const fs = require('fs');
const render = require('@jakmsok-react-pdf/render');
const pdfkit = require('@jakmsok-react-pdf/pdfkit');
